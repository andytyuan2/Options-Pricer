...
while dict['risk-free rate'] < 4: 
    u = math.exp((dict['risk-free rate'] - dict['dividend'])*dict['years']/dict['time steps'] + dict['sigma']*math.sqrt(dict['years']/dict['time steps']))
    d = math.exp((dict['risk-free rate'] - dict['dividend'])*dict['years']/dict['time steps'] - dict['sigma']*math.sqrt(dict['years']/dict['time steps']))
...
#########################################################################################################################################################################
        ...
        return discounting1
    if binomial()[0] >= 0:
        Prices_different_rates.append(round(binomial()[0], 5))
    else:
