...
def binomial():
    ...
    for x in reversed(range(1, Tstep+1)):
#####################################################################################################################################################################################
    # not used in the actual calculation but useful to see what the probabilities of each node is at a specific timestep    
        ...
        pascal = []
        for i in range(x+1):
            pascal.append(combos(x, i))

        probabilities = []
        for i in range(x+1):
            probabilities.append(pascal[i]*(probup**((x)-i))*((1-probup)**i))
#####################################################################################################################################################################################
        discounting1 = []
        for i in range(0,x):
            if dict['AmerEu'] == 1:
                American_payoff = (dict['callput']*(dict['price']*(u**(x-i-1)*(d**i)) - dict['strike']))
